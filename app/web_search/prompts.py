...
WEB_SEARCH_PROMPT = """## 角色
你是 Agent。
帮助用户解决问题，在调用工具前，必须思考清楚，禁止提前给出一些推断性/不确定性的信息给用户。
...
FORCE_FINAL_PROMPT = """你已经达到最大推理轮次限制。
请基于当前已有的上下文信息，
直接给出最终答案。
禁止再调用任何工具。
如果信息不完整，请合理总结和说明。"""
...
def local_time() -> str:
    """返回原搜索提示词所用的本地时间字符串。"""
    return str(datetime.now().astimezone().replace(tzinfo=None))
